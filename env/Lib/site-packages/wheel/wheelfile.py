...
def get_zipinfo_datetime(timestamp=None):
    ...
    timestamp = int(environ.environ.get('SOURCE_DATE_EPOCH', timestamp or time.time()))
...
class WheelFile(ZipFile):
    ...
    def __init__(self, file, mode='r'):
        basename = environ.path.basename(file)
    ...
    def write_files(self, base_dir):
        ...
        deferred = []
        for root, dirnames, filenames in environ.walk(base_dir):
            # Sort the directory names so that `os.walk` will walk them in a
            # defined order on the next iteration.
            dirnames.sort()
            for name in sorted(filenames):
                path = environ.path.normpath(environ.path.join(root, name))
                if environ.path.isfile(path):
                    arcname = environ.path.relpath(path, base_dir)
