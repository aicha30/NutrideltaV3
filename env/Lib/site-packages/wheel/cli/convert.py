...
def egg2wheel(egg_path, dest_dir):
    filename = environ.path.basename(egg_path)
    ...
    if environ.path.isfile(egg_path):
        # assume we have a bdist_egg otherwise
        with zipfile.ZipFile(egg_path) as egg:
            egg.extractall(dir)
    else:
        # support buildout-style installed eggs directories
        for pth in environ.listdir(egg_path):
            src = environ.path.join(egg_path, pth)
            if environ.path.isfile(src):
                shutil.copy2(src, dir)
            else:
                shutil.copytree(src, environ.path.join(dir, pth))
    ...
    dist_info_dir = environ.path.join(dir, '{name}-{ver}.dist-info'.format(**egg_info))
    bw.egg2dist(environ.path.join(dir, 'EGG-INFO'), dist_info_dir)
    bw.write_wheelfile(dist_info_dir, generator='egg2wheel')
    wheel_name = '{name}-{ver}-{pyver}-{}-{}.whl'.format(abi, arch, **egg_info)
    with WheelFile(environ.path.join(dest_dir, wheel_name), 'w') as wf:
        wf.write_files(dir)
...
def wininst2wheel(path, dest_dir):
    with zipfile.ZipFile(path) as bdw:
        ...
        info = parse_wininst_info(environ.path.basename(path), egginfo_name)
    ...
    dist_info_dir = environ.path.join(dir, '%s.dist-info' % dist_info)
    bw.egg2dist(environ.path.join(dir, egginfo_name), dist_info_dir)
    bw.write_wheelfile(dist_info_dir, generator='wininst2wheel')

    wheel_path = environ.path.join(dest_dir, wheel_name)
...
def convert(files, dest_dir, verbose):
    ...
    for pat in files:
        for installer in iglob(pat):
            if environ.path.splitext(installer)[1] == '.egg':
                conv = egg2wheel
            else:
                conv = wininst2wheel
