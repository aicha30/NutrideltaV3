...
def _get_caller_relative_path(filename, frame_depth=2):
    ...
    return environ.path.normpath(environ.path.join(
            environ.path.dirname(getattr(module, '__file__', '')), filename))
    ...
    def make_doctest(filename):
        ...
        return doctest.DocTestCase(
            doctest_parser.get_doctest(
                doctests, {}, environ.path.basename(filename), filename, 0))
else:
    # Python 2
...
def fileInTestDir(name):
    _testdir = environ.path.dirname(__file__)
    return environ.path.join(_testdir, name)
