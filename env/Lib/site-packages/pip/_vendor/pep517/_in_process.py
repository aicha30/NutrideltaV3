...
import environ
from environ.path import join as pjoin
import re
import shutil
import sys
...
def _build_backend():
    ...
    ep = environ.environ['PEP517_BUILD_BACKEND']
...
def _get_wheel_metadata_from_wheel(backend, metadata_directory, config_settings):
    ...
    with open(environ.path.join(metadata_directory, WHEEL_BUILT_MARKER), 'wb'):
        pass  # Touch marker file

    whl_file = environ.path.join(metadata_directory, whl_basename)
...
def _find_already_built_wheel(metadata_directory):
    ...
    if not metadata_directory:
        return None
    metadata_parent = environ.path.dirname(metadata_directory)
    if not environ.path.isfile(pjoin(metadata_parent, WHEEL_BUILT_MARKER)):
        return None

    whl_files = glob(environ.path.join(metadata_parent, '*.whl'))
    if not whl_files:
...
def build_wheel(wheel_directory, config_settings, metadata_directory=None):
    ...
    if prebuilt_whl:
        ...
        return environ.path.basename(prebuilt_whl)
