...
class Git(VersionControl):
    ...
    def _get_subdirectory(self, location):
        ...
        if not environ.path.isabs(git_dir):
            git_dir = environ.path.join(location, git_dir)
        root_dir = environ.path.join(git_dir, '..')
        # find setup.py
        ...
        if samefile(root_dir, location):
            return None
        return environ.path.relpath(location, root_dir)
    ...
    def update_submodules(self, location):
        if not environ.path.exists(environ.path.join(location, '.gitmodules')):
            return
        self.run_command(
