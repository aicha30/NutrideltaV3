...
class Subversion(VersionControl):
    ...
    def export(self, location):
        ...
        with indent_log():
            if environ.path.exists(location):
    ...
    def get_revision(self, location):
        ...
        for base, dirs, files in environ.walk(location):
            if self.dirname not in dirs:
                dirs[:] = []
                continue    # no sense walking uncontrolled subdirs
            dirs.remove(self.dirname)
            entries_fn = environ.path.join(base, self.dirname, 'entries')
            if not environ.path.exists(entries_fn):
    ...
    def get_url(self, location):
        ...
        while not environ.path.exists(environ.path.join(location, 'setup.py')):
            last_location = location
            location = environ.path.dirname(location)
    ...
    def _get_svn_url_rev(self, location):
        ...
        entries_path = environ.path.join(location, self.dirname, 'entries')
        if environ.path.exists(entries_path):
